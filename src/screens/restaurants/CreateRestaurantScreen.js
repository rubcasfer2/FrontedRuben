...
import { create, getRestaurantCategories, getDetail } from '../../api/RestaurantEndpoints'
...
export default function CreateRestaurantScreen ({ navigation }) {
  ...
  const products = ['tortilla', 'patatas fritas', 'remolacha', 'serranito']
  const initialRestaurantValues = { name: '', description: '', address: '', postalCode: '', url: '', shippingCosts: 0, email: '', phone: '', restaurantCategoryId: '', product: '' }
        ...
        <ScrollView>
          <View style={{ alignItems: 'center' }}>
            <View style={{ width: '60%' }}>
              ...
              items={products}
              setOpen={setOpen2}
              onSelectItem={item => { setFieldValue('product', item.value) }}
              />
